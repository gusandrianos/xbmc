...
bool CFile::Open(const CURL& file, const unsigned int flags)
{
  ...
  try
  {
    ...
    CURL authUrl(url);
    if (CPasswordManager::IsURLSupported(authUrl) && authUrl.GetUserName().empty())
    ...
    catch (CRedirectException *pRedirectEx)
    {
      ...
      if (pRedirectEx && pRedirectEx->m_pNewFileImp)
      {
        ...
        if (pNewUrl.get())
        {
          ...
          if (CPasswordManager::IsURLSupported(newAuthUrl) && newAuthUrl.GetUserName().empty())
...
bool CFile::Rename(const CURL& file, const CURL& newFile)
{
  try
  {
    ...
    if (CPasswordManager::IsURLSupported(authUrlNew) && authUrlNew.GetUserName().empty())
