...
namespace RETRO
{
  ...
  class CRenderBufferPRIME : public CRenderBufferSysMem
  {
  public:
    CRenderBufferPRIME(CRenderContext &context,
                       int fourcc,
                       int bpp);
    ~CRenderBufferPRIME() override;

    // implementation of IRenderBuffer via CRenderBufferSysMem
    bool Allocate(AVPixelFormat format, unsigned int width, unsigned int height, unsigned int size) override;
    size_t GetFrameSize() const override { return m_bo->GetStride() * m_height; }
