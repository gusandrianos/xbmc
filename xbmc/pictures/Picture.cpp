  ...
  CPictureScalingAlgorithm::Algorithm scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
  ...
  if (!ScaleImage(pixels, width, height, pitch, AV_PIX_FMT_BGRA,
                    buffer, dest_width, dest_height, dest_width * sizeof(uint32_t), AV_PIX_FMT_BGRA,
                    scalingAlgorithm))
  {
    delete[] buffer;
    result = NULL;
  ...
  CPictureScalingAlgorithm::Algorithm scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
  ...
  if (width > dest_width || height > dest_height || orientation)
  {
    ...
    {
      if (ScaleImage(pixels, width, height, pitch, AV_PIX_FMT_BGRA,
                     (uint8_t *)buffer, dest_width, dest_height, dest_width * 4, AV_PIX_FMT_BGRA,
                     scalingAlgorithm))
      {
...
bool CPicture::CreateTiledThumb(const std::vector<std::string> &files, const std::string &thumb)
{
  ...
  for (unsigned int i = 0; i < files.size(); ++i)
  {
    ...
    if (texture && texture->GetWidth() && texture->GetHeight())
    {
      ...
      if (ScaleImage(texture->GetPixels(), texture->GetWidth(), texture->GetHeight(), texture->GetPitch(), AV_PIX_FMT_BGRA,
                        (uint8_t*)scaled, width, height, width * 4, AV_PIX_FMT_BGRA))
      {
...
bool CPicture::ScaleImage(uint8_t *in_pixels, unsigned int in_width, unsigned int in_height, unsigned int in_pitch, AVPixelFormat in_format,
                          uint8_t *out_pixels, unsigned int out_width, unsigned int out_height, unsigned int out_pitch, AVPixelFormat out_format,
                          CPictureScalingAlgorithm::Algorithm scalingAlgorithm /* = CPictureScalingAlgorithm::NoAlgorithm */)
{
