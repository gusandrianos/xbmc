...
bool CInputHandling::OnAxisMotion(unsigned int axisIndex, float position, float center, float range)
{
  bool bHandled = false;

  float translatedPostion = std::min(std::max((position - center) / range, -1.0f), 1.0f);

  CDriverPrimitive positiveSemiaxis(axisIndex, center, SEMIAXIS_DIRECTION::POSITIVE, range);
  CDriverPrimitive negativeSemiaxis(axisIndex, center, SEMIAXIS_DIRECTION::NEGATIVE, range);

  bHandled |= OnAnalogMotion(positiveSemiaxis, position > center ? translatedPostion : 0.0f);
  bHandled |= OnAnalogMotion(negativeSemiaxis, position < center ? -translatedPostion : 0.0f);
