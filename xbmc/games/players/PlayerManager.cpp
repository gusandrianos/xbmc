...
void CPlayerManager::OpenPort(IInputHandler* handler,
...
void CPlayerManager::ClosePort(IInputHandler* handler)
{
  CExclusiveLock lock(m_mutex);
...
void CPlayerManager::MapDevices(const PeripheralVector& devices,
...
CGameClient* CPlayerManager::GameClient(JOYSTICK::IInputHandler* handler)
{
  CSharedLock lock(m_mutex);
...
void CPlayerManager::HardwareReset(JOYSTICK::IInputHandler *handler /* = nullptr */)
{
  CSharedLock lock(m_mutex);
...
IInputHandler* CPlayerManager::AssignToPort(const PeripheralPtr& device, bool checkPortNumber /* = true */)
{
